<template>
    <div class="testimonials">
      <h1>Testimonials</h1>
      <transition-group name="fade" tag="div" class="testimonial-list">
        <div class="testimonial" v-for="(testimonial, index) in testimonials" :key="index">
          <div class="testimonial-image" v-bind:style="{ 'animation-delay': (index * 0.2) + 's' }">
  ...
  const observer = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        testimonialSection.classList.remove('animate');
      }
    });
  });
